#' Get an UID for an upcoming insert into the database
#'
#' Given a connection and a method ("sampling" or "optimizing"), ask the 
#' database to generate a id number for use with `stanfit_insert`.
#'
#' @param conn A `DBI::DBConnection` object, the return value of a call to
#'   `DBI::dbConnect` .
#' @param method A string. "sampling" or "optimizing".
#'
#' @return id An integer that can be used with `stanfit_insert`.
#' 
#' @export
#' @examples
#' examples
...
#' @param id A preallocated UID, if one has already been generated by `get_run_id`. Otherwise, omit.
#' @param include_samples A logical, default `false`, whether or not to include
...
#' @export
stanfit_insert <- function(sf, conn, id = NULL, include_samples = FALSE, schema = "stanfit") {
  ...
  if (is.null(id)) {
    ...
    cli_alert_info("Using preallocated run id: {.bold {id}}")
  }

  cli_h3("Generating tables")

  tryCatch({
    tables <- get_table_entries(sf, id, progress = TRUE, include_samples = include_samples)
  },
  error = function(cond) {
    message(cond)
    cli::cli_alert_warning("Aborting transaction")
...
#' @export
optimizing_insert <- function(r, conn, id = NULL, schema = "stanfit") {
